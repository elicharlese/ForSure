...
import { CheckCircle, XCircle, AlertTriangle, ChevronDown, ChevronRight, FileText, Download } from "lucide-react"
import type { BatchValidationResult, BatchValidationProgress } from "../services/forsure-validator"
...
export function BatchValidationDialog({
  ...
  const startValidation = async () => {
    ...
    try {
      const validationResults = await ForSureValidator.validateFilesBatch(files, existingFiles, (progressUpdate) => {
  ...
  const generateValidationReport = (results: BatchValidationResult[], summary: any): string => {
    ...
    results.forEach((result, index) => {
      ...
      const allWarnings = [...result.fileValidation.warnings, ...result.contentValidation.warnings]
  ...
  const summary = results.length > 0 ? ForSureValidator.getBatchValidationSummary(results) : null
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[80vh] flex flex-col">
          ...
          {!isValidating && results.length === 0 && (
            <div className="flex-1 flex flex-col items-center justify-center space-y-4">
              <div className="text-center">
                <h3 className="text-lg font-medium mb-2">Ready to Validate Files</h3>
                <p className="text-muted-foreground mb-4">
                  {files ? `${files.length} file(s) selected for validation` : "No files selected"}
                </p>
                <Button onClick={startValidation} disabled={!files || files.length === 0}>
                  Start Validation
                </Button>
              </div>
            </div>
          ...
          {isValidating && (
            <div className="flex-1 flex flex-col items-center justify-center space-y-4">
              <div className="w-full max-w-md space-y-4">
                <div className="text-center">
                  <h3 className="text-lg font-medium mb-2">Validating Files...</h3>
                  {progress && (
                    <p className="text-sm text-muted-foreground">
                      {progress.current ? `Processing: ${progress.current}` : "Completing..."}
                    </p>
                  )}
                </div>
                    ...
                    <div className="p-4 space-y-2">
                      {results.map((result, index) => (
                        <Collapsible
                          ...
                          onOpenChange={() => toggleFileExpanded(result.fileName)}
                        >
                              ...
                              <div className="flex items-center gap-3">
                                ...
                                <div className="flex gap-1">
                                  ...
                                  {(result.fileValidation.warnings.length > 0 ||
                                    result.contentValidation.warnings.length > 0) && (
                            ...
                            <div className="ml-8 mt-2 space-y-2">
                              ...
                              {result.fileValidation.errors.length > 0 && (
                                <div className="text-sm">
                                  <div className="font-medium text-red-600 mb-1">File Errors:</div>
                                  <ul className="list-disc pl-5 space-y-1">
                                    {result.fileValidation.errors.map((error, idx) => (
                              ...
                              {result.contentValidation.errors.length > 0 && (
                                <div className="text-sm">
                                  <div className="font-medium text-red-600 mb-1">Content Errors:</div>
                                  <ul className="list-disc pl-5 space-y-1">
                                    {result.contentValidation.errors.map((error, idx) => (
                                      <li key={idx} className="text-red-600">
                                        {error}
                                      </li>
                                    ))}
                              ...
                              {(result.fileValidation.warnings.length > 0 ||
                                result.contentValidation.warnings.length > 0) && (
                                <div className="text-sm">
                                  <div className="font-medium text-amber-600 mb-1">Warnings:</div>
                                  <ul className="list-disc pl-5 space-y-1">
                                    {[...result.fileValidation.warnings, ...result.contentValidation.warnings].map(
                                      (warning, idx) => (
                              ...
                              {result.overallValid &&
                                result.fileValidation.warnings.length === 0 &&
                                result.contentValidation.warnings.length === 0 && (
                                  <div className="text-sm text-green-600">✓ File passed all validation checks</div>
                                )}
                            </div>
                          </CollapsibleContent>
                        </Collapsible>
        ...
        <div className="flex justify-end gap-2 pt-4 border-t">
          ...
          {results.length > 0 && <Button onClick={() => onOpenChange(false)}>Continue with Results</Button>}
        </div>
      </DialogContent>
    </Dialog>
  )
