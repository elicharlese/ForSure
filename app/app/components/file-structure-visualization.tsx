...
export function FileStructureVisualization({
  ...
  const [expandedNodes, setExpandedNodes] = useState<Set<string>>(new Set([structure.name]))
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if ((e.ctrlKey || e.metaKey) && e.key === "z" && !e.shiftKey && canUndo && onUndo) {
        e.preventDefault()
        onUndo()
      }
  ...
  const searchFileStructure = (node: FileNode, query: string, caseSensitive: boolean, parentPath = ""): string[] => {
    ...
    if (node.children) {
      node.children.forEach((child) => {
        results.push(...searchFileStructure(child, query, caseSensitive, currentPath))
      })
    }
  ...
  const saveNewItem = () => {
    if (!newItemName.trim() || !newItemPath || !newItemType || !onStructureChange) {
      cancelAddingItem()
      return
    }
    ...
    if (current.children.some((c) => c.name === newItemName)) {
      alert(`A ${newItemType} with the name "${newItemName}" already exists in this directory.`)
      return
    }
  ...
  const renderFileTree = (node: FileNode, path = "", level = 0) => {
    ...
    const isCurrentSearchResult = isSearchResult && searchResults[currentResultIndex] === currentPath
    ...
    return (
      <div key={currentPath} className={level === 0 ? "mt-2" : ""}>
        ...
        >
          ...
          {isEditing ? (
            <div className="flex-1 flex items-center">
              ...
              />
              <Button variant="ghost" size="sm" className="h-7 px-2" onClick={() => saveEditing(currentPath)}>
            ...
            <span
              ...
              onClick={node.type === "file" ? () => openFileForEditing(currentPath, node) : undefined}
            ...
            <div className="opacity-0 group-hover:opacity-100 transition-opacity flex items-center">
              {node.type === "directory" && (
                <DropdownMenu>
                  ...
                  <DropdownMenuContent>
                    <DropdownMenuItem onClick={() => startAddingItem(currentPath, "file")}>
                      <File className="h-4 w-4 mr-2" />
                      Add File
                    </DropdownMenuItem>
                    <DropdownMenuItem onClick={() => startAddingItem(currentPath, "directory")}>
              ...
              <Button variant="ghost" size="sm" className="h-7 w-7 p-0" onClick={() => deleteNode(currentPath)}>
          ...
          <div className="ml-4 pl-2 border-l border-dashed border-muted-foreground/20 mt-1">
            <div className="flex items-center py-1 px-1 rounded-md bg-muted/30">
              ...
              <div className="flex-1 flex items-center">
                ...
                />
                <Button variant="ghost" size="sm" className="h-7 px-2" onClick={saveNewItem}>
                  <Save className="h-3.5 w-3.5" />
                </Button>
                <Button variant="ghost" size="sm" className="h-7 px-2" onClick={cancelAddingItem}>
                  <Trash className="h-3.5 w-3.5" />
                </Button>
              </div>
            </div>
        ...
        {node.description && isExpanded && level === 0 && (
          <div className="ml-4 pl-2 mt-1 text-sm text-muted-foreground">{node.description}</div>
        )}
      </div>
    )
  }
    ...
    <div className={cn("rounded-md border p-4", className)}>
      <div className="flex items-center justify-between mb-2">
        ...
        <div className="flex gap-2">
          ...
          <Button variant="outline" size="sm" onClick={downloadAsForSureFile} className="h-8">
      ...
      <div className="bg-muted/30 rounded-md p-2">{renderFileTree(structure)}</div>
      {openFile && (
